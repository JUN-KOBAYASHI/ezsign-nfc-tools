...
| --- | --- | --- |
| `+5V`   | `+5V` | 電源 |
| `+3.3V` | `3.3V` | ほとんどの PN5180 モジュールは両方の電源が必要 |
| `GND`   | `GND` | |
| `RST`   | `G38` | リセット |
| `NSS`   | `G5`  | SPI チップセレクト |
| `MOSI`  | `G8`  | SPI |
...
> ライブラリの代替手段として、**スケッチに同梱**するのが最もシンプルです。
...
仕様書通り 250 バイトのフラグメントを使うと、APDU 全長 は 257 バイト
...
| 画像が鏡像で表示 | `sendImageIndices()` の `flipH` パラメータを反転 (true ⇔ false) |
| 画像がずれて表示 | カードが対応する以外のサイズで画像を送っている。`getDeviceInfo()` の width/height を使う |
| 2.9-inch で横向きに伸びた画像が表示 | 90 度回転が必要。`AutoSize.ino` を参考に回転ロジックを実装 |
...
- `examples/AutoSize/` — `getDeviceInfo()` でサイズ自動検出 (2.9/4.2-inch 両対応)
